...
import { URL_DEFAULT_SEARCH_URL, URL_BASE_HASHTAG, SEARCH_RETURN_COUNT, PROXY_SERVER } from './InstaProxyActions';
import { getCubeFaces, popOutImages, popInImages } from '../../components/3d/rubix/Cube';
import { 	DEFAULT_PAGING_1, DEFAULT_PAGING_2, DEFAULT_PAGING_3,
	DEFAULT_PAGING_4, DEFAULT_PAGING_5, DEFAULT_PAGING_6 } from './env';
...
function* fetchData(action) {
	try {
		...
		// If we didn't get any latest posts, try getting top posts
		if(posts.length === 0) {
		...
		if (pageInfo.end_cursor && pageInfo.end_cursor !== null) {
	...
	try {
		...
		for (i = 0; i < action.value.pages; i++) {
			...
			if (nextPage === '') {
				...
				newAction.value.searchUri = queryPath + '?tag=' + newAction.value.searchValue.toLowerCase();
...
/**
 *  Make multiple calls using environment var DEFAULT_PAGING options,
 *    then dispatch each returned set back to the store 
 */
function* fetchDefaultPages(action) {
	try {
		let i = 0;
		if (!action.value.pages) {
			return false;
		...
		for (i = 0; i < action.value.pages; i++) {
			let newAction = {...action };
			newAction.value.searchValue = defaultSearchValues[i];
			newAction.value.face = faces[i];
			let queryPath = URL_BASE_HASHTAG;
			newAction.value.searchUri = queryPath + '?tag=' + newAction.value.searchValue.toLowerCase();
...
function* instaProxySaga() {
	...
	// Special paging request if service call's recent posts data is empty
	yield takeLatest('HASHTAG_FETCH_DEFAULT_PAGING_REQUESTED', fetchDefaultPages);
}
